...
open class CropperViewController: UIViewController, Rotatable, StateRestorable, Flipable {
    ...
    open lazy var topBar: UIView = {
        let topBar = TopBar(frame: CGRect(x: 0, y: 0, width: self.view.width, height: 44 + barHeight))
    ...
    open lazy var toolbar: UIView = {
        let toolbar = Toolbar(frame: CGRect(x: 0, y: 0, width: self.view.width, height: 34 + barHeight))
...
// MARK: - Private Methods
    ...
    open func resetToDefaultLayout() {
        ...
        topBar.frame = CGRect(x: 0, y: 0, width: view.width, height: 44 + barHeight)
        toolbar.size = CGSize(width: view.width, height: 34 + barHeight)
        ...
        let topHeight = topBar.isHidden ? 44 : topBar.height
        let toolbarHeight = toolbar.isHidden ? 34 : toolbar.height
